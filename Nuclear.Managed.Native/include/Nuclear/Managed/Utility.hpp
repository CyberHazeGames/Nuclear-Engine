...
namespace Nuclear::Managed {
	...
	constexpr ManagedType GetManagedType()
	{
		...
		else if constexpr (std::is_same_v<TArg, std::string> || std::is_same_v<TArg, Nuclear::Managed::String>)
	...
	/*
	 * TODO(Emily): Work out a way to allow method invoke to use C++-y types (i.e. `std::string` instead of `Nuclear::Managed::String`).
