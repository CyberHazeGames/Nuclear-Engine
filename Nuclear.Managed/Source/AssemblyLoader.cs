...
public static class AssemblyLoader
{
	...
	private static readonly int NUCLEAR_MANAGED_ALC_CACHE_ID = -1;
	private static readonly AssemblyLoadContext? s_BridgeAssemblyLoadContext;
	...
	static AssemblyLoader()
	{
		...
		s_BridgeAssemblyLoadContext = AssemblyLoadContext.GetLoadContext(typeof(AssemblyLoader).Assembly);
		s_BridgeAssemblyLoadContext!.Resolving += ResolveAssembly;

		s_AssemblyCache.Add(NUCLEAR_MANAGED_ALC_CACHE_ID, new());

		CacheBridgeAssemblies();
	}

	private static void CacheBridgeAssemblies()
	{
		foreach (var assembly in s_BridgeAssemblyLoadContext!.Assemblies)
		{
			int assemblyId = assembly.GetName().Name!.GetHashCode();
			s_AssemblyCache[NUCLEAR_MANAGED_ALC_CACHE_ID].Add(assemblyId, assembly);
		}
	}
	...
	internal static Assembly? ResolveAssembly(AssemblyLoadContext? InAssemblyLoadContext, AssemblyName InAssemblyName)
	{
		try
		{
			...
			if (s_BridgeAssemblyLoadContext != null)
			{
				foreach (var assembly in s_BridgeAssemblyLoadContext.Assemblies)
				{
